...
import { LogOut, ArrowRight, Mail } from "lucide-react";
...
const AddEmailForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate("/emailimportant");
      }, 1500);
    } catch (err) {
      setError("حدث خطأ أثناء حفظ الايميل");
    ...
    <div className="bg-white rounded-xl p-4 md:p-8 max-w-2xl mx-auto">
      ...
      {success && (
        <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-6 text-right">
          <span className="block sm:inline">تم حفظ الايميل بنجاح</span>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            <span className="ml-2">حفظ الايميل</span>
